...
public class Cubes4dim {
    ...
    public boolean[][][][] createStartGrid() {
        ...
        int wLength = 1 + 2* addSpace;
        boolean[][][][] startGrid = new boolean[length][length][zLength][wLength];
        for (int i = 0; i < input.length; i++) {
            for (int j = 0; j < input.length; j++) {
                startGrid[addSpace +i][addSpace +j][addSpace][addSpace] = input[i][j];
            }
        }
        return startGrid;
    }
    ...
    public boolean hasExactly2or3ActiveNeighbours(boolean[][][][] grid, int x, int y, int z, int w) {
        int xLength = grid.length;
        int yLength = grid[0].length;
        int zLength = grid[0][0].length;
        int wLength = grid[0][0][0].length;
        int count = 0;
        for (int i = -1; i < 2; i++) {
            ...
            for (int j = -1; j < 2; j++) {
                ...
                for (int k = -1; k < 2; k++) {
                    ...
                    for (int l = -1; l < 2; l++) {
                        if (w+l < 0 || w+l >= wLength) continue;
                        if (x+i==x && y+j==y && z+k==z && w+l==w) continue;
                        if (grid[x+i][y+j][z+k][w+l]) count++;
                    }
                }
            }
        }
    ...
    public boolean hasExactly3ActiveNeighbours(boolean[][][][] grid, int x, int y, int z, int w) {
        int xLength = grid.length;
        int yLength = grid[0].length;
        int zLength = grid[0][0].length;
        int wLength = grid[0][0][0].length;
    ...
    public boolean[][][][] simulateOneCycle(boolean[][][][] input) {
        ...
        int wLength = input[0][0][0].length;
        boolean[][][][] output = new boolean[xLength][yLength][zLength][wLength];
            ...
            for (int j = 0; j < yLength; j++) {
                for (int k = 0; k < zLength; k++) {
                    for (int l = 0; l < wLength; l++) {
                        if (input[i][j][k][l]) {
                            if (hasExactly2or3ActiveNeighbours(input,i,j,k,l)) {
                                output[i][j][k][l] = true;
                            } else {
                                output[i][j][k][l] = false;
                            }
                        } else {
                            if (hasExactly3ActiveNeighbours(input,i,j,k,l)) {
                                output[i][j][k][l] = true;
                            } else {
                                output[i][j][k][l] = false;
                            }
